...
2. Estudiar si los datos reflejan que el proceso es meritocrático o si, por el contrario, reflejan algún tipo de injusticia
    - Esto lo haremos realizando un análisis exploratorio de datos
    - Y además, en [Experimento Adicional], dividiremos los datos en dos conjuntos: uno con variables meritocráticas y otro con variables no meritocráticas. Veremos cuál de los dos modelos tiene mejor rendimiento
    - Si el modelo no meritocrático tuviese mejor rendimiento, podríamos pensar que los datos muestran una clara injusticia en el proceso de selección
...
# Para hacer buen plot de la matriz de confusion
library(scales)
```
...
Veamos ahora las desviaciones típicas de los datos:

```{r}
sapply(df_train[, -9], sd)
```
...
**Primer método**, **Regla de Abdi et al. (2010)**: Se promedia las varianzas explicadas por la componentes principales y se selecciona aquellas cuya proporción de varianza explicada supera la media ^[Esto lo tomo directamente del código visto en las prácticas]:
...
Hacemos lo mismo para el conjunto de test. Usamos los parámetros aprendidos sobre entrenamiento para evitar hacer *data snooping*:
...
1. Consideraremos tres modelos: discriminante lineal, discriminante cuadrático y *XGBOOST*^[Se puede consultar más información sobre este modelo en la [documentación oficial](https://xgboost.readthedocs.io/en/stable/index.html)]. Tenemos que elegir, al final, cuál de los tres modelos funciona mejor (aunque entrenaremos y validaremos los tres)
...
3. *XGBOOST* tiene algunos hiperparámetros que podemos explorar. En esta parte realizaremos una exploración manual (realmente, con uno de los primeros hiperparámetros explorados fue suficiente). En el [Experimento Adicional] realizamos una exploración automática de los hiperparámetros de *XGBOOST*
...
Empezamos viendo el **accuracy** del modelo, tanto en entrenamiento como en *test*. Aunque nos interesa sobre todo el rendimiento en *test*, viendo también el rendimiento en entrenamiento podemos ver ciertos problemas (i.e. *overfitting*). Además, usaremos esa función también para calcular la matriz de confusión. Empezamos con esta función auxiliar, para no repetir el mismo código múltiples veces:

```{r}
# Calcula algunas metricas relevantes de un modelo sobre un conjunto de datos:
#   - Accuracy
#   - Matriz de confusion
...
# Se basa en otra funcion que copio completamente
plot_confusion_matrix <- function(conf_matrix) {
    ggplotConfusionMatrix(conf_matrix)
}

# Funcion que copio completamente de:
# https://www.dataenergy.biz/post/2022-03-09-plot-confusion-matrix-in-r/
...
3. No es un modelo demasiado fiable, principalmente, porque los valores fuera de la diagonal ($159 + 80 = 239) está muy cercana a los valores de la diagonal (85 + 271 = 356)
4. El modelo tiende a clasificar como no empleables (165 positivos vs 431 negativos, lo que supone una relación 27%-73%, cuando sabíamos que hay aproximadamente 40% positivos 60% negativos en nuestros datos)
...
Vamos a realizar un pequeño ajuste de los hiperparámetros de *XGBOOST* sobre ambos conjuntos de datos. Será una búsqueda de hiperparámetros usando la técnica de *Grid Search*^[Uso este [tutorial](https://www.projectpro.io/recipes/tune-hyper-parameters-grid-search-r) para implementar Grid Search]:

Mostramos el código pero no mostramos los resultados del proceso, porque se muestran demasiados mensajes durante el proceso, que hacen ilegible el cuaderno. Más adelante mostramos los resultados finales, que contienen suficiente información sobre el proceso realizado.
...
Ahora pasamos a mostrar los resultados, empezando por el conjunto meritocrático:
...
1. Aunque tenga un peor **accuracy** en entrenamiento, en *test* (que es lo que realmente nos importa), el modelo no meritocrático supera la modelo meritocrático
2. La diferencia es pequeña, algo por encima de $0.02$
3. Sin embargo, esto nos indica, **de forma contundente**, que los datos con los que estamos trabajando son en cierta medida injustos, pues no se corresponden con un proceso meritocrático
4. La diferencia con el modelo completo es aproximadamente del $0.2$. Esto es realmente poco, teniendo en cuenta que:
    1. Trabajamos con la mitad de las variables
    2. No tenemos ciertas variables que deberían ser muy relevantes
5. Obtenemos un gran rendimiento sin conocer:
    - El rendimiento académico
    - Las habilidades de comunicación y de presentar ideas
    - LA agilidad mental
6. Con solo conocer las formas de hablar, condición física, apariencia y confianza obtenemos mejores resultados que conociendo las otras variables, que deberían ser contundentemente más relevantes
7. Por tanto, podemos concluir que los datos reflejan la falta de meritocracia a la hora de encontrar un nuevo puesto de trabajo
