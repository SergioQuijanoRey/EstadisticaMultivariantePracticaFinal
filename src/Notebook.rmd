...
Además, siempre usaremos las siguientes formulas:
...
form_pca <- as.formula("CLASS ~ PC1 + PC2")

# form_fa <- as.formula("CLASS ~ MR1 + MR2")
```
...
```{r}
number_rounds <- 3
param <- list(max.depth = 2, eta = 0.3, nthread = 2, nrounds = number_rounds, objective = "binary:logistic")
```
...
# Mostramos la media de los dos rounds de cross validation
mean(results$evaluation_log$test_error_mean)
```

```
# TODO -- borrar esta celda
# Mejor accuracy: 0.4248084
# Mejores parametros:
#   number_rounds <- 3
#   param <- list(max.depth = 2, eta = 0.3, nthread = 2, nrounds = number_rounds, objective = "binary:logistic")
```
...
| Discriminante Cuadrático | Original | 0 |
| Discriminante Cuadrático | PCA      | 0 |
| Discriminante Cuadrático | FA       | 0 |
