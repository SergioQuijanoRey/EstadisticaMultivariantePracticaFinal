...
# Convertimos los datos al formato que necesita xgboost
# Usamos una formula para indicar que no se use la variable objetivo
xgboost_train <- model.matrix(CLASS~.-1, data = as.data.frame(df_train))
...
# `model` modelo del que calculamos el accuracy
get_accuracy <- function(input_vars, labels, model) {
...
Calculamos ahora los valores:

```{r}
# Calculamos las predicciones
# test_predictions <- predict(modelo_lda, df_test[, 1:8])
# test_labels <- df_test[, 9]
#
# # Miramos la cantidad de predicciones correctas
# correct_predictions <- test_predictions$class == test_labels$CLASS
# test_accuracy <- sum(correct_predictions) / length(correct_predictions)
# test_accuracy

train_accuracy <- get_accuracy(df_train[, 1:8], df_train$CLASS, modelo_lda)
test_accuracy <- get_accuracy(df_test[, 1:8], df_test$CLASS, modelo_lda)
